<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="keywords" content="" />
    <meta name="description" content="Ruleta-Falabella">
    <meta name="author" content="ZEUS COLMENARES">
    <title>Ruleta-Falabella</title>
    <link id="googleFonts" href="https://fonts.googleapis.com/css?family=Poppins:300,400,500,600,700&display=swap"
        rel="stylesheet" type="text/css">
    <link rel="stylesheet" href="{{ asset('frontend/css/bootstrap.min.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/fontawesome.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/animate.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/owl-carousel.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/owl-theme.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/theme.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/custom.css') }}?v={{ time() }}">
    <link rel="stylesheet" href="{{ asset('frontend/css/_variables.css') }}?v={{ time() }}">
    @include('frontend.layouts.ico')
    @vite(['resources/js/frontend.js'])
    @stack('styles')
</head>
<body>
    <img src="{{asset('frontend/img/elementos/1.png')}}" alt="girnalda" class="girnalda-1 overflow-hidden">
    <img src="{{asset('frontend/img/elementos/campana.png')}}" alt=campana" class="campana-1 overflow-hidden">
    {{-- <img src="{{asset('frontend/img/elementos/1.png')}}" alt="girnalda" class="girnalda-2 overflow-hidden"> --}}
    <img src="{{asset('frontend/img/elementos/campana.png')}}" alt="campana" class="campana-2 overflow-hidden">
    <img src="{{asset('frontend/img/elementos/1.png')}}" alt="girnalda" class="girnalda-3 overflow-hidden">
    {{-- <img src="{{asset('img/logos/Falabella-Logo.png')}}" alt="Logo" class="logo overflow-hidden"> --}}
    <div class="body">
        <div role="main" class="main" >
            @include('frontend.layouts.header')
                @yield('content')
        </div>
    </div>
    @if (session('success') || session('message'))
    <x-alerts.swal-notification icon="{{session('icon')??''}}" title="{{session('titulo')}}" text="{{session('message') ?? session('success') }}" timer="3000" />
    @endif
    @if (session('error') || $errors->any())
    @php
    $errorText = session('error') ?? implode(', ', $errors->all());
    @endphp
    <x-alerts.swal-notification icon="{{session('icon')??''}}" title="{{session('titulo')}}" text="{{ $errorText }}" timer="10000"/>
    @endif
    {{-- <script src="{{ asset('plugins/jquery/jquery.min.js') }}"></script> --}}
    <script src="{{ asset('frontend/js/plugins.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('frontend/js/theme.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('frontend/js/view.contact.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('frontend/js/skin.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('frontend/js/theme.init.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('plugins/jquery-validation/jquery.validate.min.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('plugins/jquery-validation/additional-methods.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('plugins/jquery-validation/localization/messages_es_PE.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('plugins/jquery-validation/init-validation.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('plugins/custom.js') }}?v={{ time() }}"></script>
    @stack('scripts')
</body>

</html>
